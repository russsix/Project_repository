...
from one_state import fetch_visa_status_data
        ...
        if response.status_code == 200:
            ...
            if 'VF' in data.get('status', ''):
                ...
                if st.button("See Visa-Free Destinations"):
                    ...
                    response = requests.get(url)
                    data = response.json()
                    visa_required_country = [get_country_name(code) for code in data.get('vr', {}).get('data', [])]
                    st.write(visa_required_country)
